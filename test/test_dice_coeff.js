...
const { diceCoeff } = require('../src');

describe('Dice Coefficient', () => {
  const stringOne = 'batman';
  const stringTwo = 'cowgirl';
  const stringThree = 'batmen';
  const n = 2;
  const over = 100;
  ...
  it(`should return 0.6 for ${stringOne} and ${stringThree} with n = ${n}`, () => {
    const result = diceCoeff(stringOne, stringThree, n);
