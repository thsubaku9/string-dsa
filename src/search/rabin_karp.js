function pow(base, exponent) {
  let final_output = 1; let internal_base = base; let
    internal_exponent = exponent;
  while (internal_exponent > 0) {
    if (internal_exponent % 2 == 0) {
      internal_base *= internal_base;
      internal_exponent = parseInt(internal_exponent / 2, 10);
    } else {
...
function rabin_karp(searchTerm, searchSpace, polynomial = 7, bucket = 3000) {
  ...
  let super_hash = 0; let current_hash = 0;
