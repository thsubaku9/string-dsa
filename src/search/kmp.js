function kmp(searchTerm, searchSpace, uniqueToken = '\b') {
  ...
  const borderSize = [];
  const textArray = searchTerm + uniqueToken + searchSpace;
  borderSize.push(0);

  let border = 0;
  for (let i = 1; i < searchTerm.length + searchSpace.length + 1; i++) {
...
function run_kmp(searchTerm, searchSpace, uniqueToken = '\b') {
  ...
  const startIndex = [];
  const correctPositions = kmp(searchTerm, searchSpace, uniqueToken);
  for (let i = 0; i < correctPositions.length; i++) {
    if (correctPositions[i] === searchTerm.length) {
      // we were offsetting the value by and additional searchTerm.length + 1
      // Hence the i - (searchTerm.length -1 ) - (searchTerm.length + 1)
